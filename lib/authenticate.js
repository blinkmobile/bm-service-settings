...
function authenticate (
  ...
  const token = auth[1]
  if (!token) {
    throw Boom.unauthorized('Please provide the correct authentication credentials', 'Bearer')
  }
  ...
  return new Promise((resolve, reject) => {
    jwt.verify(token, secret, {
      ...
      if (error) {
        reject(Boom.unauthorized('Please provide the correct authentication credentials', 'Bearer', error)) // 401
        return
      }
      resolve(data)
    })
